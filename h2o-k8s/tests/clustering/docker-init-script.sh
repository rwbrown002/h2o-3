...
k3d --version
k3d delete
k3d create -v "$H2O_BASE":"$H2O_BASE" --registries-file registries.yaml --publish 8080:80 --api-port localhost:6444 --server-arg --tls-san="127.0.0.1" --wait 120 --server-arg '--kube-proxy-arg=conntrack-max-per-core=0' --agent-arg '--kube-proxy-arg=conntrack-max-per-core=0' --enable-registry
for img in harbor.h2o.ai/library/everpeace/curl-jq:latest \
  ...
  new_name=$(echo $img | perl -pe 's#harbor.h2o.ai/library#localhost:5000#g')
  docker pull $img
